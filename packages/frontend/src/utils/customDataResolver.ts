...
export function resolveItemDisplay(item: {
  ...
  const displayType = cd && isString(cd.displayType) ? cd.displayType : item.category;
  const displayRarity = cd && isString(cd.displayRarity) ? cd.displayRarity : item.quality;
  const rawDisplayStats = cd && cd.displayStats !== undefined ? cd.displayStats : item.stats;
  const rawDisplayEffects = cd && cd.displayEffects !== undefined ? cd.displayEffects : item.effects?.map(e => `${e.type}: ${e.value}`);
